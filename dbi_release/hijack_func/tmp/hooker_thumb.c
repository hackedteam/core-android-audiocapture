  ...
  if( cblk_tmp == NULL ) {
	...
	if( lseek(cblk_tmp->fd, 0, SEEK_CUR) >= 5242880 ) {
	  ...
	  *(filename+strlen(filename)-3) = 't';
	  ...
	  filenameLength = 21 + 2 + strlen(timestamp) + strlen(cblk_tmp->trackId) + 1 + 4 + 1 + 3;

	  cblk_tmp->filename = malloc( sizeof(char) *  filenameLength  );
	  snprintf(cblk_tmp->filename, filenameLength, "/data/local/tmp/dump/Qi-%s-%s-r.bin",timestamp, cblk_tmp->trackId );
	  log("new filename: %s\n", cblk_tmp->filename);
...
/*     cblk_tmp->filename = malloc( sizeof(char) *  filenameLength  ); */
/*     snprintf(cblk_tmp->filename, filenameLength, "/data/local/tmp/dump/Qi-%s-%s-l.bin",timestamp, randString ); */
/*     log("filename: %s\n", cblk_tmp->filename); */
  ...
  if( cblk_tmp != NULL ) {
	...
	if( lseek(cblk_tmp->fd, 0, SEEK_CUR) >= 5242880 ) {
	  ...
	  *(filename+strlen(filename)-3) = 't';
	  ...
	  cblk_tmp->filename = malloc( sizeof(char) *  filenameLength  );
	  snprintf(cblk_tmp->filename, filenameLength, "/data/local/tmp/dump/Qi-%s-%s-l.bin",timestamp, cblk_tmp->trackId );
	  log("new filename: %s\n", cblk_tmp->filename);
